...
class QuadraticInvariant:
    """Represents a quadratic invariant: ax² + bxy + cy² + dx + ey + f ≤ 0 (or ==)"""
    
    def __init__(self, quad_coeffs, linear_coeffs, constant, inv_type, strength_score=0):
        """
        quad_coeffs: dict mapping (var1, var2) tuples to coefficients
                     e.g., {('x', 'x'): a, ('x', 'y'): b, ('y', 'y'): c}
        linear_coeffs: dict mapping var to coefficient {var: d, ...}
        constant: constant term f
        inv_type: "==" or "<="
        """
    ...
    def _normalize(self):
        """Normalize coefficients by GCD and ensure canonical ordering"""
        from math import gcd
        from functools import reduce
        ...
        # Ensure cross terms have canonical ordering: (x,y) not (y,x)
        normalized_quad = {}
    ...
    def format(self):
        """Format as mathematical expression"""
        terms = []
...
class RankingFunction:
    ...
    def format(self):
        expr = ""
        # Start with bound if present and non-zero
        ...
        for var in sorted(self.coeffs.keys()):
            ...
            if expr == "":
                # first term - include sign if negative
...
def sample_reachable_states(loop: Loop, max_steps: int = 30) -> List[Dict[str, int]]:
    """Simulate the loop concretely to collect reachable states."""
    states = []
    state = {v: loop.init[v] for v in loop.variables}
...
def generate_quadratic_invariants_sampling(loop: Loop, is_equality: bool, max_candidates: int = 5) -> List[QuadraticInvariant]:
    """Fallback generator: sample reachable states and solve linear system for quadratic coefficients."""
    ...
    # Build linear solver over integer coefficients
    s = Solver()
    ...
    # Build constraints from samples: for equality invariants, require expr == const; for inequalities, expr <= const
    for st in samples:
        expr = const
...
def quadratic_template(vars):
    """Create template for quadratic invariants"""
    quad_coeffs = {}
...
def quadratic_initialization_constraint(quad_coeffs, linear_coeffs, constant, loop, is_equality):
    """Check if invariant holds at initialization"""
    lhs = 0
...
def compute_quadratic_strength(inv: QuadraticInvariant, loop: Loop) -> int:
    """Compute strength score for quadratic invariant"""
    score = 0
    
    # Strong preference for equalities
    if inv.type == "==":
        score += 2000
    ...
    # Prefer fewer terms (simpler invariants)
    ...
    # Penalize large constant
    score -= abs(inv.constant) * 5
    
    # Bonus for having both quadratic and linear terms (more interesting)
    if num_quad_terms > 0 and num_linear_terms > 0:
        score += 300
    
    # Bonus for pure quadratic forms
    if num_quad_terms > 0 and num_linear_terms == 0:
        score += 200
...
def generate_quadratic_invariants(loop: Loop, is_equality: bool, 
    ...
    """Generate quadratic invariants using constraint solving"""
    s = Solver()
    s.set("timeout", 30000)  # 30 second timeout for non-linear solving
    
    quad_coeffs, linear_coeffs, constant = quadratic_template(loop.variables)
    
    # Bound coefficients to keep search tractable
    ...
    # At least one quadratic term must be non-zero
    s.add(Or([quad_coeffs[k] != 0 for k in quad_coeffs.keys()]))
    
    # Add initialization and preservation constraints
    ...
    while len(candidates) < max_candidates and attempts < max_attempts:
        ...
        if s.check() == sat:
            ...
            if verify_quadratic_invariant(inv, loop):
                # Then filter out trivial invariants
                if is_globally_valid_quadratic(inv):
                    # Skip this invariant - it's trivially true
                    blocking = []
                ...
                # Check if it's violated by some relaxation (meaningful invariant)
                if not check_violated_by_relaxation(inv, loop):
                    # This invariant might be too weak/trivial
    ...
    # If nonlinear solving didn't find any candidates, try sampling-based fallback
    if len(candidates) == 0:
...
def verify_quadratic_invariant(inv: QuadraticInvariant, loop: Loop) -> bool:
    """Verify quadratic invariant by symbolic and concrete checking"""
...
def is_globally_valid_quadratic(inv: QuadraticInvariant) -> bool:
    """
    Check if a quadratic invariant is globally valid (holds for all integer values).
    Returns True if the invariant is trivial/globally valid and should be rejected.
    """
    ...
    # Check if the negation of the invariant is satisfiable
    # If NOT satisfiable, then the invariant holds for all values (globally valid)
    s = Solver()
    s.set("timeout", 5000)  # 5 second timeout
    
    if inv.type == "==":
        # For equality, check if lhs != constant is unsatisfiable
        s.add(lhs != inv.constant)
    else:  # "<="
        # For inequality, check if lhs > constant is unsatisfiable
        s.add(lhs > inv.constant)
    
    result = s.check()
    
    # If unsat, the invariant is globally valid (trivial)
    if result == unsat:
        return True
    
    # Additional heuristic checks for common trivial forms
    
    # Check for negative definite quadratic forms with non-positive constant
    # E.g., -(x-y)² ≤ 0 is always true
    if inv.type == "<=" and inv.constant >= 0:
        # Check if all eigenvalues of quadratic form are non-positive
        # Simple heuristic: check if diagonal terms are all non-positive
        ...
        if all_non_positive and inv.constant >= 0:
            # Additional check: verify with concrete values
            # If it holds for a range of test values, likely trivial
            test_points = [
...
def check_violated_by_relaxation(inv: QuadraticInvariant, loop: Loop) -> bool:
    """
    Check if the invariant would be violated if we relaxed the loop bounds.
    Good invariants should exclude some states that are reachable under weaker constraints.
    Returns True if the invariant is meaningful (violated under relaxation).
    """
    s = Solver()
    s.set("timeout", 5000)
    ...
    # Build invariant expression
    inv_expr = inv.to_z3(z3_vars)
    
    # Try to find a state that:
    # 1. Violates the invariant
    # 2. Is "near" reachable states (within 2x the guard bounds)
    
    # Parse guard to extract bounds
    guard_str = str(loop.guard)
    relaxed_constraints = []
    
    for var in loop.variables:
        # Look for bounds like "x < 10" in guard
        import re
        # Match patterns like "x < number" or "x > number"
        matches = re.findall(rf'{var}\s*<\s*(\d+)', guard_str)
        if matches:
            bound = int(matches[0])
            # Relax to 2x the bound
        ...
        # Add reasonable absolute bounds if no specific bounds found
        if not re.search(rf'{var}\s*[<>]', guard_str):
            relaxed_constraints.append(z3_vars[var] >= -100)
            relaxed_constraints.append(z3_vars[var] <= 100)
    
    # Check if there exists a point within relaxed bounds that violates invariant
    ...
    # If SAT, we found a state that violates the invariant
    # This means the invariant is NOT trivially true - it's meaningful
    return result == sat

def synthesize_quadratic_invariants(loop: Loop, max_total: int = 5) -> List[QuadraticInvariant]:
    """Main function to synthesize quadratic invariants"""
    all_invariants = []
...
def generate_dafny_code(loop: Loop, invariants: List[Invariant], ranking_fn: Optional[RankingFunction], quad_invariants: List[QuadraticInvariant] = None) -> str:
    ...
    def _fix_bool_lists(s: str, op: str) -> str:
        # Replace patterns like '&&(a, b, c)' with '(a && b && c)'
    ...
    if ranking_fn:
        ...
        # Heuristic: if guard has a simple bound like `var < N` and that var is incremented in the body,
        # suggest `decreases N - var` so Dafny has a termination metric.
        try:
            import re
...
# Add quadratic test loops
    ...
    for i, code in enumerate(test_loops, 1):
        ...
        if ranking_fn:
            ...
            # Try the same heuristic used during generation to see if we can produce a decreases clause
            try:
                import re
                guard_s = str(loop.guard)
                m = re.search(r"([A-Za-z_][A-Za-z0-9_]*)\s*<\s*(\d+)", guard_s)
